...
const mod = await import(pathToFileURL(path.join(root, '.opencode/plugins/dci.js')).href);
const plugin = await mod.DCIPlugin({});
...
function countBootParts(out) {
  return out.messages[0].parts.filter((part) => part.type === 'text' && part.text.includes('DCI_BOOT_CONTRACT')).length;
}
