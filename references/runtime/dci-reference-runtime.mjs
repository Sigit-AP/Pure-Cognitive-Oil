...
const RELATED_START = "<!-- DCI-RELATED-START -->";
const RELATED_END = "<!-- DCI-RELATED-END -->";
...
export function renderRoute(plan) {
  const lines = [];
  lines.push("# DCI Reference Runtime Route");
  ...
  lines.push("");
  lines.push("## Use");
  lines.push(`node references/runtime/dci-reference-runtime.mjs context ${JSON.stringify(plan.query || "")} --limit ${plan.options.limit} --depth ${plan.options.depth}`);
  lines.push("dci references route \"<task>\"");
  return lines.join("\n");
}
export function buildContext(query = "", options = {}) {
  ...
  const lines = [];
  lines.push("<DCI_REFERENCE_RUNTIME_CONTEXT>");
  ...
  for (const item of plan.files.slice(0, maxFiles)) {
    const n = item.node;
    lines.push(`\n<DCI_REFERENCE path="references/${n.path}" folder="${n.folder}" axes="${n.axes.join(",")}" reason="${item.reason}">`);
    lines.push(n.text.trim());
    lines.push("</DCI_REFERENCE>");
...
export function runCli(argv = process.argv.slice(2)) {
  ...
  if (command === "folders") {
    const rt = loadReferenceRuntime();
    console.log("# DCI Reference Runtime Folders");
