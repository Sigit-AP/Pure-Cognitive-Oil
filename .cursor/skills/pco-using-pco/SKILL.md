---
name: using-pco
description: Use when starting any serious task or whenever PCO resources may apply; bootstraps deterministic cognitive routing before action.
---

<EXTREMELY-IMPORTANT>
If there is even a 1% chance any PCO resource applies, use PCO before responding or acting.
...
2. PCO skills/resources define HOW to think and execute.
3. Default model habits are lowest priority.

## Mandatory Startup

PCO starts with an AMT-native boot contract, not blind copying:
...
3. **Modifikasi** — make it PCO-specific: executable graph routing, evidence gates, full-read coverage, and runtime audit.
...
Generated `.pco/cache/*.json` files are compatibility artifacts only. Do not route from JSON when the executable runtime is available.
...
6. Execute with the PCO phase pipeline.
...
| "I can answer first" | PCO runtime route check comes before output. |
| "I'll inspect later" | Context selection is the first step. |
| "One file is enough" | PCO is graph-based; expand by neighbors and dependencies. |
