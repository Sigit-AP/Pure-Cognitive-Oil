/**
 * Pure Cognitive Oil plugin for OpenCode.ai.
 *
 * Injects PCO bootstrap context via message transform and registers PCO skills.
...
async function loadRuntimeSummary(repoRoot) {
  ...
  const runtimePath = path.join(repoRoot, 'references', 'runtime', 'pco-reference-runtime.mjs');
  ...
  try {
    ...
    runtimeCache = `\n\n**PCO Executable Reference Runtime Loaded:**\n- script: references/runtime/pco-reference-runtime.mjs\n- graph: references/reference-graph.mjs\n- files: ${plan.runtime.totals.files}\n- sections: ${plan.runtime.totals.sections}\n- edges: ${plan.runtime.totals.edges}\n\nFolder runtimes:\n${folders}\n\nStartup route:\n${selected}\n\nRuntime commands:\n- pco references route \"<task>\"\n- pco references capsule \"<task>\"\n- pco references context \"<task>\" --limit 8 --depth 1\n- pco agentic-auto \"<task>\"`;
    return runtimeCache;
  } catch (err) {
    runtimeCache = `\n\n**PCO Executable Reference Runtime:** failed to import; run node references/runtime/pco-reference-runtime.mjs route \"<task>\". Error: ${err instanceof Error ? err.message : String(err)}`;
    return runtimeCache;
  }
}

export const PCOPlugin = async () => {
...
When PCO references tools you do not have, substitute OpenCode equivalents:
- \`TodoWrite\` → \`todowrite\`
...
Use PCO by loading graph context first, selecting relevant nodes, traversing cross-layer relations, and verifying through quality-safety gates.`;

    bootstrapCache = `<PCO_BOOT_CONTRACT>
You have Pure Cognitive Oil.

**IMPORTANT: PCO bootstrap is already loaded. Do not reload it redundantly. Operate through the PCO graph and verification gates.**
...
- Modifikasi: use PCO's executable graph, runtime audit, full-read coverage, and quality/safety gates instead of copying another framework.
...
${toolMapping}
  ...
  return {
    ...
    'experimental.chat.messages.transform': async (_input, output) => {
      ...
      if (firstUser.parts.some((p) => p.type === 'text' && p.text.includes('Pure Cognitive Oil'))) return;
