...
export function checkDciHealth() {
  ...
  const checks = [];
  add(checks, "bootstrap script exists", exists("scripts/dci/bootstrap.mjs"));
  add(checks, "bootstrap contract marker exists", contains("scripts/dci/bootstrap.mjs", "DCI_BOOT_CONTRACT"));
  ...
  add(checks, "OpenCode plugin caches bootstrap", contains(".opencode/plugins/dci.js", "bootstrapCache"));
  add(checks, "OpenCode plugin dedupes injection", contains(".opencode/plugins/dci.js", "DCI_BOOT_CONTRACT") || contains(".opencode/plugins/dci.js", "Deterministic-Cognitive-Infrastructure"));
...
export function formatHealthReport(report) {
  const lines = ["DCI_HEALTHCHECK", `ok: ${report.ok}`, `runtime: files=${report.runtime.files}, sections=${report.runtime.sections}, edges=${report.runtime.edges}`, `scorecard: ${report.scorecard.ratio}x valid=${report.scorecard.valid}`, ""];
