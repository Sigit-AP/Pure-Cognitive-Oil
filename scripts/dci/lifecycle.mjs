...
import {
  ...
  routeReferences,
  searchReferences,
} from "../../references/runtime/dci-reference-runtime.mjs";
...
export function buildLifecycleCertificate() {
  ...
  try {
    ...
    routeOutput = run("references/runtime/dci-reference-runtime.mjs", ["route", "mid execution drift verification", "--limit", "8", "--depth", "1"]);
  ...
  try {
    const bootEnvelope = JSON.parse(run("scripts/dci/bootstrap.mjs", ["--json"]));
  ...
  const runAll = exists("tests/dci/run-all.sh") ? read("tests/dci/run-all.sh") : "";
  const validateSource = exists("scripts/dci/validate.ts") ? read("scripts/dci/validate.ts") : "";
  ...
  const firstUse = phase("first-use", "The framework must activate as a boot contract before an agent starts acting.", [
    check("bootstrap-script", exists("scripts/dci/bootstrap.mjs"), "scripts/dci/bootstrap.mjs exists"),
    check("boot-contract-marker", bootContext.includes("DCI_BOOT_CONTRACT"), "bootstrap emits DCI_BOOT_CONTRACT"),
    ...
    check("route-command", bootContext.includes("dci references route") && bootContext.includes("references/runtime/dci-reference-runtime.mjs"), "bootstrap includes executable route commands"),
  ...
  const midUse = phase("mid-use", "The framework must resist drift after work has already started.", [
    ...
    check("context-command-present", contextOutput.includes("<DCI_REFERENCE_RUNTIME_CONTEXT>") && contextOutput.includes("mid execution drift"), `${contextOutput.length} chars of mid-use context generated`),
  ...
  const runtimePhase = phase("runtime", "The framework must route and adapt from executable graph state, not static prose only.", [
    check("runtime-main", exists("references/runtime/dci-reference-runtime.mjs"), "main reference runtime exists"),
  ...
  const finalUse = phase("final-use", "The framework must finish with evidence, validation, and repo hygiene instead of a vague done claim.", [
    ...
    check("runtime-audit-script", exists("scripts/dci/runtime-audit.mjs"), "runtime audit script exists"),
    check("healthcheck-script", exists("scripts/dci/healthcheck.mjs"), "healthcheck script exists"),
    check("scorecard-script", exists("scripts/dci/scorecard.mjs"), "2x scorecard script exists"),
    ...
    check("package-lifecycle-script", typeof scripts["dci:lifecycle"] === "string" && scripts["dci:lifecycle"].includes("lifecycle.mjs"), "package exposes dci:lifecycle"),
    check("package-claim-scripts", ["dci:healthcheck", "dci:runtime-audit", "dci:scorecard"].every((name) => typeof scripts[name] === "string"), "package exposes healthcheck, runtime-audit, and scorecard"),
    check("package-all-runs-lifecycle", typeof scripts["dci:all"] === "string" && scripts["dci:all"].includes("dci:lifecycle"), "dci:all includes lifecycle gate"),
    ...
    check("cache-ignored", gitignore.includes(".dci/cache/"), "generated DCI cache is ignored"),
  ...
  return {
    ...
    target: {
      ...
      claimBoundary: "Measures executable DCI lifecycle coverage against a one-shot bootstrap baseline; it does not claim universal correctness.",
    },
    ...
    evidence: {
      ...
      commands: [
        ...
        "node references/runtime/dci-reference-runtime.mjs route \"<task>\"",
        "node references/runtime/dci-reference-runtime.mjs context \"<task>\"",
        "node scripts/dci/lifecycle.mjs",
        "node scripts/dci/healthcheck.mjs",
        "node scripts/dci/scorecard.mjs",
        "npm test",
      ],
      outputFiles: [".dci/cache/lifecycle-certificate.json"],
    },
  };
}
...
export function formatLifecycleCertificate(report) {
  ...
  lines.push(`DCI lifecycle ${report.status}: score=${report.score}, sustainedRatio=${report.target.sustainedLifecycleRatio}`);
  return lines.join("\n");
}
