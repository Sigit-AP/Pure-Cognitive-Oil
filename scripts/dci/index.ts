...
export function buildAgentRouting(m: DciManifest): AgentRouting {
  ...
  routes.push({ trigger:"any-professional-task", axes:AXES, requiredFiles:core, optionalFiles:m.files.map(f=>f.path), concepts:m.concepts.slice(0,80).map(c=>c.term), instruction:"Mandatory DCI mode: load startup core, select workflow by task, select cognitive engines by axis, expand with graph neighbors and concept map, verify with quality/safety files before final." });
