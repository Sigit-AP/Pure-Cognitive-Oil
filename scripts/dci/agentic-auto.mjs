...
import { routeReferences, buildCapsule } from "../../references/runtime/dci-reference-runtime.mjs";
...
function buildInterviewSkill(query, risk, complexity) {
  ...
  return {
    active,
    purpose: "Only for /DCI-agentic-auto: extract real intent, constraints, success criteria, risk tolerance, hidden preferences, and unacceptable outcomes before action.",
    rules: [
...
const lifecycle = run("node", ["scripts/dci/lifecycle.mjs", "checkpoint", task]);
const validate = flags.has("--verify") ? run("npm", ["run", "dci:validate"]) : null;
...
const lines = [];
lines.push("# /DCI-agentic-auto Full Autonomous Runtime Contract");
...
lines.push("");
lines.push("## Interview skill — active only for /DCI-agentic-auto");
...
lines.push("3. ROUTE: select DCI workflows/engines/gates/context depth from task evidence.");
