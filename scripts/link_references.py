#!/usr/bin/env python3
"""Build and validate the PCO reference knowledge graph.
...
- A "Related PCO references" block in every reference .md.
"""
...
def build_graph() -> dict:
    ...
    for rel, meta in nodes.items():
        ...
        for t in peers[:2]:
            edges[rel].add(t)
            relation_meta.append({"from": rel, "to": t, "type": "same-folder-peer", "purpose": "preserve local continuity inside the same PCO subsystem"})
    ...
    return {"nodes": nodes, "edges": edges, "relations": relation_meta, "runtime_contract": {"agent_entry": "SKILL.md", "human_index": "references/REFERENCE_GRAPH.md", "machine_graph": "references/reference-graph.mjs", "runtime": "references/runtime/pco-reference-runtime.mjs", "regenerator": "scripts/link_references.py", "rule": "Load executable graph first, choose relevant nodes, traverse cross-layer edges, then verify with quality-safety nodes."}}
...
def update_docs(graph: dict) -> None:
    for rel, targets in graph["edges"].items():
        ...
        lines = [START, "", "## Related PCO references", ""]
        for t in targets:
            lines.append(f"- [{graph['nodes'][t]['title']}]({rel_link(rel, t)})")
        lines += ["", END]
        block = "\n".join(lines)
        legacy_patterns = [(START, END)]
        replaced = False
        for old_start, old_end in legacy_patterns:
            if old_start in text and old_end in text:
                text = re.sub(re.escape(old_start) + r".*?" + re.escape(old_end), block, text, flags=re.S)
                replaced = True
                break
        if not replaced:
            text = text + "\n\n" + block
...
def write_indexes(graph: dict) -> None:
    ...
    for rel, meta in graph["nodes"].items():
        by_cat.setdefault(meta["category"], []).append(rel)
    out = ["# PCO Reference Graph", "", "Executable graph: [`reference-graph.mjs`](reference-graph.mjs). Runtime loader: [`runtime/pco-reference-runtime.mjs`](runtime/pco-reference-runtime.mjs).", ""]
...
def validate(graph: dict) -> int:
    ...
    if errors:
        print("PCO reference graph validation failed:")
        print("\n".join(errors))
        return 1
    print(f"PCO reference graph valid: {len(graph['nodes'])} nodes, {sum(len(v) for v in graph['edges'].values())} edges")
    return 0
