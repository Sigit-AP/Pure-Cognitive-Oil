...
- references/reference-graph.json: machine-readable graph for plugins/agents.
...
def build_graph() -> dict:
    ...
    return {"nodes": nodes, "edges": edges, "relations": relation_meta, "runtime_contract": {"agent_entry": "SKILL.md", "human_index": "references/REFERENCE_GRAPH.md", "machine_graph": "references/reference-graph.json", "regenerator": "scripts/link_references.py", "rule": "Load graph first, choose relevant nodes, traverse cross-layer edges, then verify with quality-safety nodes."}}
...
def write_indexes(graph: dict) -> None:
    (REFS / "reference-graph.json").write_text(json.dumps(graph, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    by_cat = {}
    for rel, meta in graph["nodes"].items():
        by_cat.setdefault(meta["category"], []).append(rel)
    out = ["# DCI Reference Graph", "", "Machine-readable graph: [`reference-graph.json`](reference-graph.json).", ""]
