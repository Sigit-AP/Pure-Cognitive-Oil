...
function listFiles(dir) {
  ...
  function walk(d) {
    for (const ent of fs.readdirSync(d, { withFileTypes: true })) {
      if ([".git", ".pco", "node_modules", "dist"].includes(ent.name)) continue;
...
function contentSearch(pattern) {
  ...
  return listFiles(".")
    .filter((file) => file !== "scripts/pco/runtime-audit.mjs")
...
function stripRelated(text) {
  const start = text.indexOf("<!-- PCO-RELATED-START -->");
  const end = text.indexOf("<!-- PCO-RELATED-END -->");
...
function summarizeReferenceFile(file, graph, runtime) {
  ...
  return {
    ...
    relatedBlock: text.includes("<!-- PCO-RELATED-START -->") && text.includes("<!-- PCO-RELATED-END -->"),
...
if (!exists("references/runtime/pco-reference-runtime.mjs")) fail("runtime-main", "missing references/runtime/pco-reference-runtime.mjs");
...
try {
  runtimeModule = await import(pathToFileURL(path.join(root, "references/runtime/pco-reference-runtime.mjs")).href);
...
const routeText = run(["references/runtime/pco-reference-runtime.mjs", "route", "debug hallucination verification", "--limit", "8", "--depth", "1"]);
...
const contextText = run(["references/runtime/pco-reference-runtime.mjs", "context", "debug hallucination verification", "--limit", "8", "--depth", "1", "--max-chars", "12000"]);
...
try {
  const boot = JSON.parse(run(["scripts/pco/bootstrap.mjs", "--json"]));
  const ctx = boot.additionalContext || boot.additional_context || boot.hookSpecificOutput?.additionalContext || "";
  if (ctx.includes("PCO_BOOT_CONTRACT") && ctx.includes("AMT-derived but PCO-native") && ctx.includes("references/runtime/pco-reference-runtime.mjs")) pass("first-use-boot-contract", `${ctx.length} chars`);
  else fail("first-use-boot-contract", "bootstrap context missing PCO boot contract, AMT marker, or runtime command");
} catch (err) {
...
if (exists("scripts/pco/bootstrap.mjs") && !exists("scripts/pco/bootstrap.ts") && !/npx\s+tsx|bootstrap\.ts/.test(sessionHook)) pass("zero-dependency-bootstrap", "session hook uses node bootstrap.mjs without npx/tsx");
...
const allowedJsonRefFiles = new Set(["scripts/pco/runtime-audit.mjs", "scripts/link_references.py"]);
...
try {
  const { PCOPlugin } = await import(pathToFileURL(path.join(root, ".opencode/plugins/pco.js")).href);
  const plugin = await PCOPlugin({});
  const output = { messages: [{ info: { role: "user" }, parts: [{ type: "text", text: "Build safely" }] }] };
  await plugin["experimental.chat.messages.transform"]({}, output);
  await plugin["experimental.chat.messages.transform"]({}, output);
  const injectedParts = output.messages[0].parts.filter((part) => part.type === "text" && part.text.includes("Pure Cognitive Oil"));
...
fs.mkdirSync(path.join(root, ".pco/cache"), { recursive: true });
fs.writeFileSync(path.join(root, ".pco/cache/runtime-audit-report.json"), JSON.stringify(report, null, 2) + "\n");
fs.writeFileSync(path.join(root, ".pco/cache/reference-full-read-report.json"), JSON.stringify(report.referenceFullRead, null, 2) + "\n");

for (const check of checks) console.log(`${check.status.toUpperCase()} ${check.name}: ${check.detail}`);
if (warnings.length) console.log(`WARNINGS: ${warnings.length}`);
if (failures.length) {
  console.error(`PCO runtime audit fail: ${failures.length} failures`);
  process.exit(1);
}
console.log(`PCO runtime audit pass: ${checks.filter((c) => c.status === "pass").length}/${checks.length} checks`);
