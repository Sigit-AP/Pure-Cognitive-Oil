...
export function buildAudit(m: DciManifest): AuditReport {
  ...
  const folderCoverage=Object.entries(m.folders).map(([folder,v])=>{ const infrastructureFolder = ["bin", "hooks", "scripts", "skills", "tests"].some(prefix => folder === prefix || folder.startsWith(`${prefix}/`)); const hasCoverage = v.files>0&&v.axes.length>0&&(v.concepts.length>0||infrastructureFolder); return {folder,files:v.files,axes:v.axes.length,concepts:v.concepts.length,status:(hasCoverage?"pass":"fail") as "pass" | "fail"}; });
