...
const lifecycleReportPath = path.join(root, ".pco", "cache", "lifecycle-certificate.json");
if (!fs.existsSync(lifecycleReportPath)) errors.push("missing lifecycle certificate: run npm run pco:lifecycle");
else {
...
if (errors.length) {
  console.error("PCO validation failed:");
  for (const e of errors) console.error(`- ${e}`);
  process.exit(1);
}
console.log(`PCO validation pass: files=${manifest.totals.files}, sections=${manifest.totals.sections}, concepts=${manifest.totals.concepts}, edges=${manifest.totals.edges}, score=${audit.readinessScore}`);
