---
description: Full autonomous PCO mode with adaptive interview, route, plan, delegate/parallel execution, build, audit, repair, and optimization loops.
argument-hint: [task]
...
You are running PCO in **full autonomous optimal end-to-end mode** for:
...
Run the PCO runtime contract first:

```bash
pco agentic-auto "$ARGUMENTS"
```

If `pco` is not linked, run from the PCO repo root:

```bash
node scripts/pco/agentic-auto.mjs "$ARGUMENTS"
```
...
3. **ROUTE** — use PCO route/capsule to select workflows, engines, references, gates, and context depth.
